...
## 2026-09-12 v9.1.3 Web反映・キャッシュ対策
- 症状: 新ZIPを公開してもWeb側で旧UI（「イラストとGIFを作る」、旧保存/共有ボタン）が表示されることがあった。
- 原因区分: ファイル転送失敗ではなく、ブラウザ/Service Worker/中間キャッシュによる旧資産の残留。
- 対応: Service Worker登録解除、`oekaki-jump-*` Cache Storage削除、静的資産URLのバージョン付与、移行用自己解除Service Workerを追加。
- 再発防止: UIリリースごとに静的資産のバージョン文字列を更新し、Service Workerによる長期キャッシュを使用しない。
